/* core/app.js — applyLang, listeners, hash router, nav, header-scroll, reveal */
  ...
  function applyLang(){
    ...
    /* aria-labels must switch language with the page too — a JA screen-reader user was
       hearing "Close", "Menu", "Filters" in English while html lang said ja */
    document.querySelectorAll("[data-i18n-aria]").forEach(function(n){ var k=n.getAttribute("data-i18n-aria"); var v=I18N[lang][k]!=null?I18N[lang][k]:I18N.en[k]; if(v!=null) n.setAttribute("aria-label", v); });
  ...
  function hydrateFromApi(){
    ...
    /* Articles hydrate the same way (independent, silent on failure): admin-managed
       posts — AI-drafted, human-published — replace the baked placeholders. A row
       needs a bilingual title and body paragraph arrays; a missing language falls
       back to the other so a half-translated post never renders an empty modal.
       Invalid rows are skipped; an empty/failed fetch keeps the baked posts. */
    grab("/api/articles").then(function(list){
      if(!Array.isArray(list) || !list.length) return;
      try{
        var rows=[];
        ...
        list.forEach(function(a){
          ...
          var bodyEn=a.body.en.filter(isStr), bodyJa=a.body.ja.filter(isStr);
          if(!bodyEn.length && !bodyJa.length) return;   /* a card with an empty reading modal is worse than the placeholder */
        ...
        ARTICLES=rows;
        renderArticles();
      }catch(e){ if(window.console&&console.error) console.error("[articles hydrate]", e); }
    });
  }
