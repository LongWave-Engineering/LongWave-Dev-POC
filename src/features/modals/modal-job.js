/* features/modals/modal-job.js — the job-detail (JD) modal: fill, open, and the
  ...
  function hydrateJob(job){
    ...
    job._jdLoading = true;
    /* Clearing the flag BEFORE the re-render is load-bearing: fillJobModal reads it to
       decide between "Loading…" and "Not listed (N/A)", so leaving it set here left every
       genuinely-empty section (e.g. Language requirement) saying "Loading…" forever —
       caught on the live board, not by any test. Re-render on failure too, for the same
       reason. */
    var settle = function(){
      job._jdLoading = false;
    ...
    fetch("/api/jobs/"+encodeURIComponent(job.id))
      ...
      .catch(function(){ settle(); /* offline or 404 — sections fall back to "Not listed" */ });
  }
