  ...
  function renderCompanies(){
    ...
    if (window.__HRMOS_DATA__ && window.__HRMOS_DATA__.JOBS && window.__HRMOS_DATA__.JOBS.length) {
      ...
      Object.keys(_counts).sort(function(a,b){ return _counts[b]-_counts[a]; }).forEach(function(co){
        ...
        card.innerHTML=
          ...
          '<div class="co-actions"><button class="btn btn--primary" data-go="jobs" data-co-search="'+ esc(c.name) +'">'+ t("comp_view") +'</button>'+
          (c.site?'<a class="btn btn--ghost" href="'+ esc(c.site) +'" target="_blank" rel="noopener">'+ t("m_company") +'</a>':'')+'</div>';
    ...
    COMPANY_LIST.forEach(function(co){
      ...
      if(live>0){
        ...
        feat.innerHTML=
          '<span class="avatar" style="background:'+ esc(co.color) +';width:64px;height:64px;font-size:1.4rem">'+ esc(co.mono) +'</span>'+
          ...
          '<div class="co-actions"><button class="btn btn--primary" data-go="jobs" data-co-search="'+ esc(co.name) +'">'+ t("comp_view") +'</button>'+
          (co.site?'<a class="btn btn--ghost" href="'+ esc(co.site) +'" target="_blank" rel="noopener">'+ t("m_company") +'</a>':'')+'</div>';
        grid.appendChild(feat);
      } else {
        var card=el("div","co-card");
        card.innerHTML=
          '<div class="co-head"><span class="avatar" style="background:'+ esc(co.color) +'">'+ esc(co.mono) +'</span>'+
