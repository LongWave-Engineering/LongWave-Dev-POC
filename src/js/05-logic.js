/* [build] 05-logic.js  (pure, framework-free domain logic) - concatenated in load order by build.ps1; all parts share one IIFE scope.
   This module has NO DOM and NO app state, so the exact same code is:
     • used by the browser bundle (exposed as `LW` in the IIFE scope), and
     • require()'d by the Node unit tests in /test (via the module.exports footer).
   Keep anything that's testable pure logic here; keep DOM/render code elsewhere. */
  var LW = (function(){
    "use strict";
